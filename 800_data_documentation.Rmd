...
To install the add-in, run this script (note that it needs the package called {remotes}, which you might not have).
...
Now restart RStudio, and the functions of {annotater} will appear in the Addins menu.
...
library(fs) # Cross-Platform File System Operations Based on 'libuv', CRAN v1.5.0 

fs::dir_tree(path = here::here(), recurse = TRUE)

dir_tree()

dir_tree(here::here("lessons", "class_05"))
...
A blog article with some examples of how the functions within {fs} can be used: https://www.tidyverse.org/blog/2018/01/fs-1.0.0/
